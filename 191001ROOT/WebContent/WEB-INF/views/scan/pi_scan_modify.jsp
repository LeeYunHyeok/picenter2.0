...
// console.log(aTypeList.split(","));
// for (var i = 0; i < aTypeList.length; i += 1) 
// {
// 	console.log(aTypeList[i].DATATYPE_ID);
// }
...
function searchViewScan()
{
	...
	$.ajax({
	    ...
	    success: function (policyList) {
	    	...
	    	$("#policyName").val(policy.policy_name);
	    	$("#policyVer").val(policy.policy_version);
    		$("#policyComment").val(policy.comment);
    		$("#scanLabel").val(policy.schedule_label);

    		if (policy.schedule_pause_from != "08:00") {
    			var pauseFrom = policy.schedule_pause_from;
    			pauseFrom = pauseFrom / 60;
    			var startMinute = pauseFrom % 60;
    			var startHour = (pauseFrom - startMinute) / 60
    			var nFromTime = startHour + ":" + startMinute + "0"
    			console.log(nFromTime);
    			$("#pauseStartTime").val(nFromTime);
    		}

    		if (policy.schedule_pause_to != "20:00"){
    			var pauseTo = policy.schedule_pause_to;
    			pauseTo = pauseTo / 60;
    			var startMinute = pauseTo % 60;
    			var startHour = (pauseTo - startMinute) / 60
    			var nToTime = startHour + ":" + startMinute + "0"
    			console.log(nToTime);
    			$("#pauseEndTime").val(nToTime);
    		}	

    		if (policy.schedule_cpu == "normal"){
    			$("#PriorityLow").prop("checked", true);
    		}
    		
    		var throughput = policy.schedule_data;
    		if (throughput != null){
    			$("#chkThroughput").prop("checked", true);
    			$("#throughput").prop('disabled', false);
    			$("#throughput").val(throughput);
    		}
    		
    		var memory = policy.schedule_memory;
    		if (memory != null){
    			$("#chkMemory").prop("checked", true);
    			$("#memory").prop('disabled', false);
    			$("#memory").val(memory);
    		}
    		
    		var trace = policy.schedule_trace;
    		if (trace){
    			$("#chkTrace").prop("checked", true);   			
    		}
			
    		$("#limitDay").val(policy.dmz);
	    	$("#defaultPolicy").prop("checked", true);
    		

    		
    		
    		var sDatatype = policy.datatype_id;
    		var split = sDatatype.split(',');
    		
    		
    		var arr = $('[name=chkDatatype]').val();
    		console.log(arr);
    		
			for (var i = 0; i < split.length; i++) {
				console.log(split[i]);
				if (split[i] == $('[name=chkDatatype]').val()){
					//var id_by_name = $('[name=chkDatatype]').attr('id');
					//alert(id_by_name);
				}
			}
